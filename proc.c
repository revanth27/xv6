...
int totals[5] = {-1, -1, -1, -1, -1}, q_ticks_max[5] = {1, 2, 4, 8, 16};

void update_flag(struct proc *p)
{
  acquire(&ptable.lock);
  p->jump = 1;
  release(&ptable.lock);
...
void update_qticks(struct proc *p)
{
  acquire(&ptable.lock);
  p->ticks[p->queue]++;
  release(&ptable.lock);
...
int push(struct proc *p, int q_no);
int pop(struct proc *p, int q_no);
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    {
      ...
      for (hp = ptable.proc; hp < &ptable.proc[NPROC]; hp++)
      {
        ...
        if (hp->priority < selected_proc->priority)
        {
            founder = 1;
            break;
        }
      ...
      if (p->priority == selected_proc->priority && p->state == RUNNABLE)
      {
  ...
  for (;;)
  {
    ...
    if (selected_proc != 0 && selected_proc->state == RUNNABLE)
    {
  ...
  for (;;)
  {
    ...
    for (int i = 1; i < 5; i++)
    {
      ...
      {
        struct proc *lp = queue[i][j];
        int age = ticks - lp->enter;
        if (age > 30)
        {
          pop(lp, i);
          push(lp, i - 1);
...
int push(struct proc *p, int num)
{

  for (int i = 0; i < totals[num]; i++)
  {
  ...
  p->queue = num;
  totals[num]++;
  queue[num][totals[num]] = p;
  return 1;
}

int pop(struct proc *p, int num)
{
  for (int i = 0; i <= totals[num]; i++)
  {
    if (queue[num][i]->pid == p->pid)
    {
      for (int j = i; j < totals[num]; j++)
        queue[num][j] = queue[num][j + 1];
      totals[num] -= 1;
      break;
    }
  }
...
int cps(void)
{
  ...
  //int flag_found = 0;
  cprintf("PID \t Pri- \t State \t        r_time \tw_time  n_run \t cur_q \t q0 \t q1 \t q2 \t q3 \t q4\n");
  cprintf("    \t ority\n");
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    #if (SCHEDULER == S_PBS || SCHEDULER == FCFS || SCHEDULER == S_RR)
      p->queue = -1;
      ...
      if (p->state == SLEEPING)
      {
        cprintf("%d \t %d \t SLEEPING \t %d \t %d \t %d \t %d \t %d \t %d \t %d \t %d \t %d\n", p->pid, p->priority, p->rtime, p->wait_time, p->num_run, p->queue, p->ticks[0], p->ticks[1], p->ticks[2], p->ticks[3], p->ticks[4]);
      }
